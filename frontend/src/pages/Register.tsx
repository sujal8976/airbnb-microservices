...
export function Register() {
  ...
  return (
    <div className="mx-auto my-12 max-w-[460px] rounded-card border border-line bg-paper-raised p-7 shadow-card">
      <h1 className="mt-0 font-display text-[1.7rem]">Create your account</h1>
      {error && (
      ...
      <form onSubmit={handleSubmit}>
        <div className="mb-4 flex flex-col gap-1.5">
          <label className="text-[0.8rem] font-semibold uppercase tracking-wide text-ink-soft">Name</label>
          <input
            value={name}
        ...
        <div className="mb-4 flex flex-col gap-1.5">
          <label className="text-[0.8rem] font-semibold uppercase tracking-wide text-ink-soft">I am a</label>
          <select
            value={role}
            onChange={(e) => setRole(e.target.value as "guest" | "host" | "both")}
            className="rounded-md border border-line-strong bg-paper-raised px-3 py-2.5 text-[0.95rem] text-ink"
          >
            <option value="guest">Guest — booking stays</option>
            <option value="host">Host — listing a place</option>
